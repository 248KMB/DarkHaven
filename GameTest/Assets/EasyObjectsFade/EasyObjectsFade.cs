...
public class EasyObjectsFade : MonoBehaviour
{
    ...
    {
        UseAllMeshRenderers, UseAllMeshRenderersAlsoInChildren, UseOneMeshRenderer,UseMeshColliders
    }
    ...
    private void FixedUpdate()
    {
        ...
        foreach (RaycastHit h in hits)
        {
            ...
            switch (meshDetectionType)
            {
                ...
                case MeshDetectionType.UseMeshColliders:
                    MeshCollider meshCollider = h.collider.GetComponent<MeshCollider>();
                    if (meshCollider != null)
                    {
                        // Handle MeshCollider visibility here
                        // For example, you can access materials using meshCollider.sharedMaterials
                        // Implement your fade logic similar to how it's done for MeshRenderers.
                    }
                    break;
            }
            ...
            {
                Debug.Log("Detected Material" + mat.name);

                if (!fadedOut.Contains(mat))
                {
                    Debug.Log("Fading Out: " + mat.name);
                    fadedOut.Add(mat);
